...
# Архитектура проекта
В данном проекте используется подход Model-View-Presenter, сокращённо MVP, позволяющий разделить отображение и данные. Также для коммуникации с сервером добавлен слой коммуникации (connector). Архитектура данного приложения состоит из трех слоев:
1. Слой отображения (View-Presenter) — интерфейс для взаимодействия с пользователем. Его задача — выводить что-то на экран и генерировать события с действиями пользователя. 
2. Слой данных (Model) — бизнес-логика проекта и хранение данных. Его задача — хранить данные и обрабатывать события на изменение этих данных.
3. Слой коммуникации (Connector) — отвечает за связь между данными и их представлением.
...
class AppState extends Model<IAppState> - класс описывающий текущее состояние сайта, а именно список товаров, доступных для заказа; текущий активный товар; список товаров, добавленных в корзину; характеристика заказа.
    ...
    Основные методы:
        ...
        fillOrder(): void; - заполняет order согласно данным корзины;
...
class BasketUI extends Component<IBasketUI> - класс, описывающий содержимое окна корзины с товарами.
    ...
    Основные методы:
        ...
        set total(total: number): void; - сеттер для суммарной цены товаров в корзине.
...
class class OrderUI extends Form<IOrderForm> - класс, описывающий содержимое, которое будет выводиться в окне заказа.
    ...
    Основные методы:
        ...
        getPayment(): string; - возвращает значение выбранного способа оплаты;
        checkValid(): boolean; - возвращает состояние валидации;
